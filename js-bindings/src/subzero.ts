    ...
    if (dbType === 'sqlite' ) {
        ...
        switch (clientType) {
            ...
            case 'wasm-sqlite3':
                ...
                (dbPool as WasmSqlite3Database).createFunction('jwt', function () {
                    return boundJwt();
...
async function restPg(dbPool: PgDatabase, subzero: SubzeroInternal,
    ...
    try {
        ...
        if (o.setDbEnv && o.wrapInTransaction) {
            ...
            if (o.debugFn) {o.debugFn('env query', envQuery, envParameters);}
            await db.query(envQuery, envParameters);
        }
        if (o.debugFn) {o.debugFn('main query', query, parameters);}
...
async function restSqlite(dbPool: SqliteDatabase, subzero: SubzeroInternal,
    ...
    if (method == 'GET') {
        ...
        if (o.debugFn) {o.debugFn('env', Object.fromEntries(queryEnv));}
        if (o.debugFn) {o.debugFn('main query', query, parameters);}
        ...
        if (o.debugFn) {o.debugFn('env', Object.fromEntries(queryEnv));}
        if (o.debugFn) {o.debugFn('mutate query', mutate_query, mutate_parameters);}
        switch (clientType) {
            case 'better-sqlite3':
                try {
                    ...
                    if (o.debugFn) {o.debugFn('select query', select_query, select_parameters);}
...
export const formatResourceLabel = (str: string):string => {
    str = str.replace(/ID$/, ' Id');
    // Split on underscore, hyphen, or camel case
            ...
            const transformedObjects = allowedObjects
                .map(({ name, kind, columns, foreign_keys }: SchemaObject) => {
                    ...
                    const transformedColumns = columns?.map((c) => {
                        return {
                            name: c.name,
                            title: formatResourceLabel(c.name),

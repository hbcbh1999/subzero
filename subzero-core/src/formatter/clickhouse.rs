...
//fmt_query!();
pub fn fmt_query<'a>(
    schema: &String, _return_representation: bool, wrapin_cte: Option<&'static str>, q: &'a Query, join: &Option<Join>,
) -> Result<Snippet<'a>> {
                ...
                Ok((
                    ...
                    vec![
                        "left join ("
                            ...
                            + join_conditions
                                .iter()
                                .map(|c| fmt_condition(&Qi("".to_string(), local_table_name.clone()), c))
                                .collect::<Result<Vec<_>>>()?

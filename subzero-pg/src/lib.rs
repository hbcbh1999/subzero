...
pub fn handle(
    schema: &str,
    relation: &str,
    request: pgrx::composite_type!(REQUEST_TYPE),
    env: &str,
    max_rows: Option<i64>
    ...
    let headers: HashMap<&str,&str> = headers_array.map(
        |a| a.iter().filter_map(|h|
            h.map(|h| (
    ...
    //info!("got env");
    let statement = fmt_main_query_internal(db_schema, schema_name, method, &accept_content_type, &query, &preferences, &_env)
        .map_err(|e| e.to_string());
    ...
    match &response {
        Ok(r) => {
            ...
            headers_map.extend(r.headers.as_ref().map(|h| serde_json::from_str::<HashMap<&str, &str>>(h).unwrap_or_default()).unwrap_or_default());
            //info!("RESPONSE: {:?} {:?} {:?}", status, headers_map, body);
            let headers: Vec<pgrx::composite_type!(HEADER_TYPE)> = headers_map.into_iter().map(|(k,v)| {
...
mod tests {
    ...
    fn test_handle() {
        ...
        let response = crate::subzero::handle(
            "public",
            "users",
            request,
            "",
        ...
        let _headers = response.get_by_name::<Vec<pgrx::composite_type!(HEADER_TYPE)>>("headers")
            .unwrap_or_default().unwrap_or_default();
        let headers = _headers.iter().map(|h| {

...
fn do_exit() {
    ...
    if let Ok(output) = Command::new("docker").arg("ps").output() {
        ...
        for line in String::from_utf8_lossy(&output.stdout).lines() {
            if line.contains("subzero-build") {
...
fn _cp_src_dir(srcdir: &PathBuf, repodir: &PathBuf) -> Result<(), std::io::Error> {
    let mut command = Command::new("cp");
    //let src = format!("{}/", srcdir.canonicalize().unwrap().display()); 
    command
        .arg("-R")
        .arg(srcdir.canonicalize().unwrap())
        .arg(repodir.canonicalize().unwrap());

    println!("{} `{}` to `{}`", "     Copying".bold().green(), srcdir.display(), repodir.display());
    let command_str = format!("{:?}", command);
    let output = command.output()?;
    handle_command_output((), command_str, &output)?;
    ...
    package_sh_target.push("docker-build-system");
    package_sh_target.push("package.sh");
    println!("{} ./package.sh to `{}`", "     Copying".bold().green(), package_sh_target.display());
    std::fs::copy("./package.sh", package_sh_target)?;
    Ok(())
}

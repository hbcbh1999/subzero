...
with
  ...
  view_col_rels as (
    select
      ...
      targetentries.attname as view_col_name,
    ...
    from
      ...
      , lateral (
          select
            ...
            matches[4]::smallint as table_col_position
          from
            regexp_matches(
              r.ev_action,
    ...
    where
      c.relkind in ('v', 'm')
      and n.nspname = any (:api_schemas::name[])
...
--     'table_table_m2o_rel_cols_agg', coalesce(table_table_m2o_rel_cols_agg.array_agg, array[]::record[]),
...
--   (select array_agg(table_table_m2o_rel_cols) from table_table_m2o_rel_cols) as table_table_m2o_rel_cols_agg,
  
-- --   (select array_agg(view_col_rels) from view_col_rels) as view_col_rels_agg,
-- --   (select array_agg(rels) from rels) as rels_agg,
--   pg_version
select * from view_col_rels limit 10;
